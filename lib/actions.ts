...
export async function createPoll(formData: FormData) {
  try {
    ...
    const hasEmptyOptions = optionsArray.some(
  ...
  try {
    ...
    if (error) {
      ...
      console.error("Vote submission failed:", {
        pollId,
        optionId,
        currentVotes: currentOption.votes,
        error,
      });
...
export async function updatePoll(updateData: {
  ...
  try {
    ...
    if (pollError) {
      ...
      console.error("Poll metadata update failed:", {
        pollId: updateData.pollId,
        pollError,
      });
    ...
    if (fetchError) {
      console.error("Failed to fetch existing options:", {
        pollId: updateData.pollId,
        fetchError,
      });
    ...
    for (const option of updateData.options) {
      if (option.id.startsWith("new_")) {
        ...
        if (insertError) {
          console.error("Failed to add new option:", {
            pollId: updateData.pollId,
    ...
    const optionsToRemove =
      existingOptions?.filter((opt) => !currentOptionIds.includes(opt.id)) ||
      [];
